...
Output is old-school email HTML: `<table role="presentation" cellSpacing="0">`, inline styles, MSO conditional comments injected via `dangerouslySetInnerHTML` (`blocks/Button/index.tsx`). `blocks/Text` sanitizes both of its rich formats through `insane` — markdown via `marked` in `EmailMarkdown.tsx`, and the canvas toolbar's inline marks in `EmailRichText.tsx` — **keep that sanitization when touching text rendering.**
...
- **Rich text** (`editor/helpers/richText/`): a Text block whose `props.format` is `'html'` is edited through `InlineEditable`'s `rich` mode — plain `contentEditable`, and `SelectionToolbar` following the selection with bold / italic / underline / strikethrough / colour. Three things are load-bearing. The toolbar **never takes focus** (every pointer press inside it is prevented): a focus change would collapse the selection before the command could run, and would blur the editable and commit the block mid-edit. Formatting is `document.execCommand`, deprecated but the only API that edits a live selection with no document model behind it — which is the point, since the rendered block _is_ the surface; `styleWithCSS` is off for the four marks so browsers emit **semantic tags**, because Word renders `<strong>/<em>/<u>/<del>` reliably but is patchy on `text-decoration` as CSS. And `normalize.ts` reconciles the browsers on commit (Chrome's `<b>` and per-line `<div>`, Firefox's spans, `rgb()` → hex) while `blocks/Text/EmailRichText.tsx` is the actual security boundary and runs again at render. `props.format` replaced the old `markdown` boolean so the three readings of `text` are exclusive by construction; the inspector rewrites the string when it switches (`richText/convert.ts`), or a literal `a < b` would become markup.
