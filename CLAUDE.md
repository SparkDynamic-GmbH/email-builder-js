...
- **Persistence**: none in the package — `onChange` is the hook for it, debounced by `onChangeDebounceMs` (trailing edge, flushed on unmount; `0` means report every change). Restoring is `initialDocument`, read once, so a host fetching from a backend must not render the provider until the fetch resolves. The host app loads from `window.location.hash` (`#sample/<name>` or `#code/<base64>`, `getConfiguration/index.tsx`) and `ShareButton` encodes back. Save status, dirty tracking and `beforeunload` flushing are still the host's.
