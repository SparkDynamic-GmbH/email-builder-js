...
packages/
  email-builder/           the one published package, @sparkdynamic/email-builder
    ...
    src/editor/            the editor library — provider, canvas, inspector, ui primitives
      ...
      i18n/                en/de/fr/it catalogs, I18nProvider, useTranslate
...
Every renderer wraps its content in that table and puts the **padding on the `td`** — Word ignores padding on a `div`. Background colour is emitted twice, as CSS and as a `bgcolor` attribute, and alignment likewise as `text-align` and an `align` attribute; Word honours the attribute where it drops the property. Two blocks go further, and the reasons are load-bearing: `Divider` draws its rule as a cell's `border-top` rather than an `<hr>` (Word renders its own and ignores the width and colour set on it), and `Spacer` holds its height with a `&nbsp;` at a matching `line-height` and a 1px font, because an empty cell collapses. An element **narrower than the block it sits in is a table of that width**, placed with an `align` attribute _and_ auto margins — Word honours the attribute and ignores `margin: auto`, every browser-based client does the opposite, and `align` on a `display: table` element moves nothing outside quirks mode. `Divider`'s `lineWidth` and `Text`'s `inlineBackground` both work this way; a span with `display: inline-block` is not an alternative, since Word collapses it back to full width. **Write new blocks this way from the start** rather than adding to a pile that has to be migrated later.

The exception is `EmailLayoutEditor` — canvas chrome, never email output, so it stays a `div` and keeps its click-to-deselect handler. It also carries `eb-canvas`, which is what makes the canvas a query container.

**Responsiveness lives in two places and has to.** `createReader` writes the exported document's whole head as a string — charset, viewport, Word's `PixelsPerInch`, a body margin reset, and a `@media` rule that stacks a `ColumnsContainer`'s cells below 600px, since a table cell cannot wrap and stacking means it stops being a cell. The canvas cannot reuse that rule: the mobile toggle narrows a _box_ to 370px inside a full-width window, so a viewport query never matches. `src/editor/styles.css` therefore repeats it as a `@container eb-email` query against the `eb-canvas` element. A host rendering the reader itself — the preview tab — has to put `eb-canvas` on its own wrapper, as `examples/…/TemplatePanel` does. Anything that must survive a client stripping `<style>` belongs inline on the block instead, the way the image block's `max-width` does.
...
- **Style presets and block defaults** (`editor/styleDefaults/`): the document's styling covers not just the layout's colours and typeface but **what a freshly inserted block starts from** — `EmailLayoutPropsSchema` carries a `blockDefaults` map keyed by block type, and `BlocksMenu` runs `resolveNewBlock` over the definition's own defaults before inserting. The merge is section-wise, `props` over `props` and `style` over `style`, so an entry carrying only `style` keeps the placeholder content the definition declares; the merged result is parsed against `registry.blockSchema` and falls back to the definition if it does not fit, which is what makes a stale or hand-edited entry harmless. Storing it on the root block rather than beside the document is deliberate: `documentSchema` is `z.record(string, blockSchema)`, so a sibling top-level `theme` key would fail validation, and this way styling travels with the JSON for free. A **preset** is that pair named; `stylePresets` on the provider is the same callback contract as the image and template libraries, defaulting to `BUILT_IN_STYLE_PRESETS` when the prop is left out. `StylePresetPanel` is the inspector's **Presets tab**, on the same terms as the Templates tab — it exists only when the library can offer or save something (`isStylePresetLibraryUsable`), and a selection left pointing at a tab whose library went away falls back to Styles. Saving hands the host a `TStylePresetDraft` and nothing else; the list on screen is the host's `presets` array, so a saved preset appears when the host puts it there. A preset marked `readOnly` shows no delete affordance, which is how a host offers a shipped set beside the user's own. `applyStylePreset` returns the whole document so the change is one undo step, keeps `preheader` and `childrenIds` because those are content, and **merges** its defaults into the document's rather than replacing them — same section-wise merge, one level up — so a preset says only what it is about and cannot take a default away, only overwrite it. Its restyle pass reads `preset.blockDefaults`, not the merged map, so it reaches the types the preset brought rather than every type the document happens to hold a default for. `BlockDefaultsPanel` edits a type's default through **that block's own `SidebarPanel`** against a stand-in block, so there is no parallel set of inputs and a host's block is covered for nothing. Both it and the Global panel are keyed on `useExternalRevision`: the panels are uncontrolled, so a write from outside them — a preset, an undo — has to remount them, and a key over the data itself would remount on every slider tick instead.
...
- **i18n** (`editor/i18n/`): every string the chrome renders comes from a flat, dotted-key catalog. `en.ts` is the source of truth — `TTranslationKey` is `keyof typeof en`, and `de.ts`, `fr.ts` and `it.ts` are typed `Record<TTranslationKey, string>`, so **adding a key fails the build until every other catalog has it too**. Write them as UTF-8 and check the accents afterwards: the style-preset strings once landed double-encoded in three catalogs at once. The provider takes `language` (ISO 639-1, default `'en'`) and `translations`, a partial override map that also accepts keys of a host's own; lookup is override → language → English → the key itself. `useTranslate()` returns `t(key, params?)`, with `{name}` placeholders. Renderers under `src/blocks/` are untouched — they render the document, which is the user's own words. Add-block labels resolve by convention under `block.<type>` and fall back to the label the definition gave, which is how a host's own block keeps a label without a catalog entry.
...
| Style presets, block defaults | `src/editor/styleDefaults/` — the contract in `types.ts`, the shipped set in `presets.ts`, the Presets tab in `StylePresetPanel.tsx`. Adding a themable field means nothing here: the panels are the blocks' own. The host app's stand-in store is `examples/vite-emailbuilder/src/stylePresets.ts` |
